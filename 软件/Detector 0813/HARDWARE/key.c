...
void key_run_main_task(void)
{
	...
	{				 
		//常规一般按键测试（按下键就起作用）：
		case KEY_EVENT(KB_KEY0,PRESS_DOWN):	//KEY2按下即有效，控制LED0翻转	
			//LCD_ShowString(10,120,24,"1",BLACK,WHITE);
			break;
		case KEY_EVENT(KB_KEY1,PRESS_DOWN):	//KEY0按下即有效，控制LED1翻转	 
			//LCD_ShowString(10,120,24,"2",BLACK,WHITE);
			break;
		case KEY_EVENT(KB_KEY2,PRESS_DOWN):	//KEY0按下即有效，控制LED1翻转	 
			//LCD_ShowString(10,120,24,"3",BLACK,WHITE);
			break;
		
		//下面可自由增加其它按键测试，比如（仅举数例）：
		case KEY_EVENT(KB_WKUP,SINGLE_CLICK):	//KEY1键单击（快速按下再松开）	 
			//LCD_ShowString(10,120,24,"4",BLACK,WHITE);
			break;
		case KEY_EVENT(KB_WKUP,LONG_RRESS_START):	 
			LCD_ShowString(81,107,24,"  POWER OFF  ",WHITE,BLACK);
			delay_ms(1000);
			LCD_Clear1(BLACK);
			VCC_EN = 0;
			break;
...
//			printf("KEY4 long\r\n");
//			break;
//		case KEY_EVENT(KB_WKUP,LONG_PRESS_HOLD):	//长按键KEY1后，每300ms处理一次（实现类似电子表的长按"+"时连续快速调整时间的功能）
//			if(Flag300ms)
//			{
//				Flag300ms=0;
