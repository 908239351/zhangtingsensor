...
* Return         : None
*******************************************************************************/
void ILI9341_DrawPoint(u16 Xpos,u16 Ypos,u16 RGB_Code)
{
...
void LCD_DrawLine(u16 sx, u16 sy, u16 ex, u16 ey,u16 cala)
{
	...
	{
		ILI9341_DrawPoint(uRow,uCol,cala);//画点 
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void LCD_Draw_Circle(u16 x0,u16 y0,u8 r,u16 RGB_Code)
{
	...
	while(a<=b)
	{
		ILI9341_DrawPoint(x0-b,y0-a,RGB_Code);
		ILI9341_DrawPoint(x0+b,y0-a,RGB_Code);
		ILI9341_DrawPoint(x0-a,y0+b,RGB_Code);
		ILI9341_DrawPoint(x0-b,y0-a,RGB_Code);
		ILI9341_DrawPoint(x0-a,y0-b,RGB_Code);
		ILI9341_DrawPoint(x0+b,y0+a,RGB_Code);
		ILI9341_DrawPoint(x0+a,y0-b,RGB_Code);
...
void LCD_DrawFullCircle(u16 Xpos,u16 Ypos,u16 Radius,u16 RGB_Code)
{
	...
	for(y=Ypos - r;y<Ypos +r;y++)
	{
		for(x=Xpos - r;x<Xpos+r;x++)
		{
			...
			{
				ILI9341_DrawPoint(x,y,RGB_Code);
			}
		}
	}
}
